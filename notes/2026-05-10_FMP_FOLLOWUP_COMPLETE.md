...
4. ✅ **Git strategy split for bot commits** — bot vs manuel commit ayrımı için standart konvansiyon uygulandı. `docs/GIT_STRATEGY.md` belgesi eklendi. 7 workflow (`agent`, `morning_scan`, `news_radar`, `adil_deger_panel`, `adil_deger_weekly`, `macro_calendar`, `telegram_bot`) tek standardize edildi: bot author `Finzora Bot <finzora-bot@users.noreply.github.com>`, manuel author `Finzora AI <zeynelgun@users.noreply.github.com>` korundu. Commit mesaj prefix'leri standartlaştırıldı: `[bot/agent-MODE]`, `[bot/scan]`, `[bot/scan-retry]`, `[bot/swing-entry]`, `[bot/radar]`, `[bot/panel]`, `[bot/panel-weekly]`, `[bot/macro]`, `[bot/telegram-offset]`. Filtre: `git log --author='finzora-bot@'` veya `git log --grep='^\[bot/'`. Geriye dönük rewrite YAPILMADI (force-push riski yüksek), kesim noktası 10 May 2026. 14 workflow YAML syntax PASS.
