...
- kural (DİNAMİK SEKTÖR BAZLI VIX MATRİSİ — v4, 6 nisan 2026):
  ...
  ⚠️ sistemik finansal krizde tüm sektörler duyarlı (bkz. kriz tipi tablosu)
- pozisyon boyutu: VIX <28 iken faydalanıcı sektörler tam pozisyon (formül uygulanmaz). VIX 28+ iken tüm sektörlerde formül: standart × (22/VIX). örnek: VIX 30'da $10K → $7.3K
- stop genişlemesi: VIX 28+ iken 2xATR → 3xATR, pozisyon başına max %3 portföy riski
- K-13b: duyarlı sektörlerde VIX 28-35'te ichimoku 4/4 istisnası (detay aşağıda)
...
- **K-13b: ichimoku 4/4 istisnası (sadece VIX-duyarlı sektörler için)**
  ...
  - istisna koşulları (TÜMÜ eşzamanlı sağlanmalı):
    ...
    2) sektör trendi aktif: hissenin sektör ETF'i hem 9SMA hem 21SMA üzerinde. ETF eşleştirmesi: XLK, XLE, XLI, XLC, XLV, XLF, XLP, XLY, XLU, XLB
    3) mcap >$2B
  ...
  - ek güvenlik katmanları (istisna kullanıldığında zorunlu):
    a) stop: chandelier exit trailing + max %5 (hangisi daha sıkıysa)
    b) max 2 eşzamanlı istisna pozisyonu (toplam risk sınırı)
    c) aynı sektörden max 1 istisna pozisyonu (korelasyon koruması)
...
**v2.3 özeti**:
- ön filtre: ichimoku 4/4 zorunlu → K-13 v4.1 sektör bazlı VIX matrisi uygula (faydalanıcı sektör VIX 28'e kadar tam, duyarlı VIX 22'den itibaren yarım) → SPY > 21SMA + eğim ↗ → K-19 → K-20. duyarlı sektörlerde VIX >28 → K-13b istisnası gerekli
