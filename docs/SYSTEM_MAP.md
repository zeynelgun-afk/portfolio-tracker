...
agent.yml tetiklenir
   ...
   │
   └─► Claude değişiklik tespit ederse:
...
agent.yml tetiklenir
   ...
   │      │     Kapanan trade'leri Claude ile analiz, closed.json lessons
   │      ├─ score_pending_predictions() → prediction_log.json
   │      └─ Darwin evrim kontrolü (5 işlem gününde 1)
   │
   ├─► Claude Opus 4.6 kapanış raporu
...
orchestrator.py ────────────────────── Ana orkestratör (entry point)
   ...
   │
   ├─ macro_intelligence.py           ⭐ TEK TEMA KAYNAĞI (Claude tespit)
...
BUG DÜZELTMELERİ:
  orchestrator _load_alpha_scan_context() kaldırıldı — Claude 9 gündür stale
