# portföy raporu — 2026-04-21

> finzora ai | sabah raporundan otomatik ayristirildi
> kaynak: DAILY_SABAH_2026-04-21.md — 3-rapor gorsel ayristirma, token maliyeti yok

## 0. Piyasa İstihbaratı

**Aktif temalar:**
- **Earnings sezonu başladı:** T ve VRT yarın açıklıyor, NEE/HIG perşembe. K-05 yoğun hafta.
- **AI altyapı momentumu:** COHR ("Ultimate AI Bottleneck Play"), MU ("Nasdaq rally pick"), ANET ("Momentum Stock") medyada öne çıkıyor. [MUHTEMEL]
- **Nükleer enerji satışı:** OKLO -%7.1, NNE -%4.9, BWXT -%4.9, CCJ -%4.8 gap aşağı. Sektör bazlı risk-off. Portföyde doğrudan etkilenen yok. [KESİN]
- **Jeopolitik rejim aktif:** XLE +%1.2 lider, XLF/XLY/XLP kırmızı. Savunmacı rotasyon sürüyor.

**Haber etki zinciri:**
- T: "alarming pattern ahead of earnings" (invezz) + yarın earnings → T üzerinde baskı riski [MUHTEMEL]
- COHR: seekingalpha "ultimate AI play, +%88 YTD" → momentum destekli ama pahalı (v5: -%40 PAH)
- MO: "dividend stock transforms $500k into $3,100/month" → retail ilgi, ama stop %1.3 uzak ⚠️

---

## 0.5. Dün Seans Sonu Notları

İlk çalışma — session_state flag yok. Dünkü brief ("VRT K-05 çıkış, MO/DUK stop yakın") bugün operasyonel plan.

---

## 2. Haber ve Analiz

**Portföyü etkileyen:**
- **T (AT&T):** "alarming pattern ahead of earnings" + yarın earnings. Temettü pozisyonu, K-05 mevcut pozisyonu etkilemez, tutulur. [KESİN]
- **VRT:** Yarın earnings. K-05 gereği bugün kapanışta zorunlu çıkış. Şu an +%9.1 kârda, stop %8.3 uzak. [KESİN]
- **COHR:** Seekingalpha "AI bottleneck play" — pozitif anlatı, +%21.1 kârda, stop %17.4 uzak. Trailing aktif. [MUHTEMEL]
- **MU:** Zacks "strong momentum" + Nasdaq rally pick. +%8.3 kârda, hedef $470. [KESİN]
- **ETN:** Q1 earnings 5 Mayıs. Şimdilik etki yok, dividend ilan etti.
- **OKE:** Zacks "earnings beat unlikely" — 28 Nisan earnings yaklaşıyor, izle. [MUHTEMEL]

**Twitter sinyalleri [SPEKÜLATİF]:** SPY 710 call wall + SPX büyük put alımları → yukarı sınır + aşağı hedge. Tek yönlü tez yok.

---

## 3. Portföy Sağlık Durumu

### Agresif $380,610 (-4.8%)
Tümü kâr, stoplar uzak. VRT bugün kapanışta K-05 ÇIKIŞ. v5 fair value çoğunlukla PAH — trailing sıkı tutulur.

### Dengeli $107,105 (+7.1%)
⚠️ **NEE stop %3.3 uzak** + 23 Nis earnings (2 gün). K-05 gereği yarın kapanış çıkış hazırlığı.
⚠️ HIG stop %5.9 uzak ama 23 Nis earnings — yarın kapanış çıkış.

### Temettü $141,864 (+41.9%) 🏆
⚠️ **MO stop %1.3 uzak** — senaryo testinde %5 piyasa düşüşünde tetiklenecek. Override yasak, tetik gelirse otomatik çık (K-06). T yarın earnings, K-05 mevcut pozisyonu etkilemez.

### Konsantrasyon
🔴 **Tech %54.6** — K-17 üst sınıra yakın. Yeni tech girişi yasak.

### Earnings Takvimi (K-05 kritik)
| Sembol | Tarih | Aksiyon |
|--------|-------|---------|
| VRT | 22 Nis (yarın) | **Bugün kapanış ÇIKIŞ** |
| T | 22 Nis (yarın) | Temettü pozisyonu — tut, K-05 etkilemez |
| NEE | 23 Nis | Yarın kapanış çıkış hazırlığı |
| HIG | 23 Nis | Yarın kapanış çıkış hazırlığı |
| VZ | 27 Nis | 5 gün — izle |
| OKE | 28 Nis | 6 gün — izle |

---

## 4. Günün Planı

### HEMEN (bugün kapanışa doğru)
1. **VRT ÇIK %100** — K-05 zorunlu çıkış, +%9.1 kârda. Kapanışa yakın uygula.

### İZLE (otomatik tetik)
2. **MO** — stop %1.3 uzak. Tetiklenirse override yasak, K-06 otomatik çıkış.
3. **NEE** — yarın kapanış K-05 çıkış hazırlığı.
4. **HIG** — yarın kapanış K-05 çıkış hazırlığı.

### PASİF
- COHR/ANET/MU/CAMT/ETN: trailing stoplar aktif, müdahale yok.
- T: yarın earnings ama temettü teması, K-05 mevcut pozisyonu etkilemez, tutulur.
- Swing 5/5 dolu, yeni giriş imkansız.

### YASAKLAR
- K-17: tech %54.6, yeni tech girişi yasak.
- K-05: earnings haftası yoğun, yeni agresif giriş yok.
- K-02: jeopolitik kriz rejimi, momentum sektör yeni giriş yasak.

---

*Rapor sonu. Tüm aksiyonlar K-kurallarına uygun, onay beklenmeden uygulanacak.*

---
