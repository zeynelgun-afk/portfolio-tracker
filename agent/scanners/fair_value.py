...
# ────────────────────────── Scanner Adaptörü (Faz 2 Adım 6) ──────────────────────────


def _evaluate_fair_value_for_candidate(
    symbol: str,
    min_potential_pct: float = 25.0,
):
    """Tek ticker için adil değer hesabı → Candidate veya None. Pure transform.

    discover_undervalued_tickers() içindeki Candidate üretim mantığının saf hali:
    AI gate çağırmaz, watchlist'e yazmaz, sadece FV/iskonto skorunu hesaplar
    ve Candidate üretir.

    Args:
        symbol: Ticker
        min_potential_pct: Eşik (default %25). Bunun altında None döner.

    Returns:
        Candidate veya None (eşik altı, veri eksik, hata durumlarında).

    Faz 2 — Adım 6 (17 May 2026).
    """
    from agent.scanners.base import Candidate

    try:
        quote_resp = fmp_get("quote", {"symbol": symbol})
        if not quote_resp or not isinstance(quote_resp, list):
            return None
        current = quote_resp[0].get("price")
        if not current or current <= 0:
            return None
        ...
        # Skor: potansiyel % → 0.0-1.0 (linear, max 1.0)
        # Tasarım dokümanı Bölüm 5: ham score, kalibratör çarpan uygular
        try:
            score = max(0.0, min(1.0, potential_pct / 100.0))
        except (TypeError, ValueError):
            score = 0.5

        reason = (
            f"FV ${fv['fair_value']:.2f} = +%{potential_pct:.1f} potansiyel "
            f"(en yeni hedef ${fv['latest_target']}, en yüksek ${fv['highest_target']}, "
            f"sample {fv['sample_size']})"
        )
        ...
        return Candidate(
            ...
            metadata={
                ...
                "highest_target": fv["highest_target"],
                "sample_size": fv["sample_size"],
                "min_threshold_pct": min_potential_pct,
            },
        )
    except ValueError:
        # Candidate yaratım hatası (boş symbol vb.)
        return None
    except Exception as e:
        print(f"[fair_value] _evaluate {symbol} hatası: {e}")
        return None


class FairValuePanelScanner:
    """BaseScanner adaptörü — Faz 2 Adım 6 (17 May 2026).

    Mevcut script mantığını (fetch_analyst_targets, compute_fair_value) yeniden
    kullanır. scan() yan etki yapmaz — sadece Candidate listesi döndürür.
    Yan etkiler (watchlist eklemesi, AI gate, raporlama) CLI main() içinde
    discover_undervalued_tickers() + update_watchlist_fair_values() üzerinden
    devam eder.
    ...
    def __init__(
        ...
        if min_potential_pct <= 0:
            raise ValueError(
                f"min_potential_pct > 0 olmalı, alındı: {min_potential_pct}"
            )
        self.min_potential_pct = float(min_potential_pct)
        self.universe = universe  # None → analist_takip evreni

    def _resolve_universe(self) -> list[str]:
        """Constructor'da verilen universe varsa onu, yoksa analist_takip evreni."""
        if self.universe is not None:
            return list(self.universe)

        try:
            from agent.legacy.analist_takip.watchlist import build_watchlist  # type: ignore
        except ImportError as e:
            print(f"[fair_value scanner] analist_takip import hatası: {e}")
            return []
        ...
        try:
            ...
            print(f"[fair_value scanner] universe oluşturulamadı: {e}")
            return []

    def scan(self) -> list:
        """Universe tara → her ticker için FV hesapla → Candidate listesi.

        Yan etki yok. AI gate burada çağrılmaz — Candidate üretildikten sonra
        kalibratör ve/veya AI Gate ayrı katmanda işlenir.
        """
        ...
        candidates: list = []
        for symbol in tickers:
            cand = _evaluate_fair_value_for_candidate(
                symbol, min_potential_pct=self.min_potential_pct
            )
            if cand is not None:
    ...
    def health_check(self) -> dict:
        ...
        return {
            ...
            "min_potential_pct": self.min_potential_pct,
